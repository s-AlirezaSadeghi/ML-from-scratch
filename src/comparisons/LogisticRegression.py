...
from src.classification.logistic_regression import LogisticRegression
...
logging.info(f"Coefficients using Sklearn model {model_sk.coef_[0]}")

logging.info(f"Biases using ml-from-scratch model {model.bias}")
logging.info(f"Biases using Sklearn model {model_sk.intercept_[0]}")
