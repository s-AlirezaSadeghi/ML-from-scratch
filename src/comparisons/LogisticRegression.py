...
model = LogisticRegression(verbose=True,learning_rate=0.05,iterations=12000)
...
print(f"Accuracy of  custom model {custom_accuracy}")
print(f"Accuracy of  Sklearn model {sk_accuracy}")


tn, fp, fn, tp = confusion_matrix(model.predict(X_test),Y_test).ravel()
print(f'Custom model True Negative: {tn} , False Positive: {fp} , False negative: {fn} , True Positive: {tp} ')
...
print(f'Dummy model True Negative: {tn} , False Positive: {fp} , False negative: {fn} , True Positive: {tp} ')
